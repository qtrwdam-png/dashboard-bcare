...
import { reconcileSelectedVisitor } from "@/lib/selection-helpers";
...
export default function Dashboard() {
  ...
  useEffect(() => {
    const unsubscribe = subscribeToApplications((apps) => {
      ...
      // Keep the selected visitor synced and clear it if Firestore deleted the record.
      setSelectedVisitor((prev) => {
        const next = reconcileSelectedVisitor(prev, sorted);
        selectedVisitorIdRef.current = next?.id || null;
        return next;
      });
    });
